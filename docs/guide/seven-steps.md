...
👉 [01 服务器上电与规划](/setup/server-onboarding)
...
👉 [03 固件与驱动安装](/setup/firmware-driver)
...
👉 [05 PyTorch + torch_npu 安装](/setup/torch-npu-install) · [06 MindSpore 安装](/setup/mindspore-install)

## 第 6 步：接入业务

- 跑起一个**验证脚本**确认环境 OK。
- 然后根据你的目标进入 [训练](/training/) 或 [推理](/inference/)。

## 第 7 步：环境自检

- 用一份**自检清单**逐项确认：设备识别、驱动状态、框架可用性、性能基线。

👉 [08 环境自检清单](/setup/checklist)
...
> [!TIP]
> 想跳过繁琐手动安装？很多场景可以直接用官方 **Docker 镜像**，一步拿到可用环境，见 [Docker 镜像与离线部署](/setup/docker-offline)。
