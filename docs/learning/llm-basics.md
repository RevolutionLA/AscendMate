...
- 算力需求与**参数量、数据量、序列长度**成正比：多模态数据训练需求约是文本数据的 **320 倍**；序列长度越长越吃显存与算力。
...
> 以下内容作为本阶段的**扩展补充**，补全大模型的定义、涌现、参数明细、类型、Token、三类网络对比与小/大模型对比。已在前面讲过的 Transformer 原理、训练流程、推理优化、Prompt、RAG/Agent 不再重复。
